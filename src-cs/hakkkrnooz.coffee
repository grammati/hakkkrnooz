...
removeFollowingColumns = (elt) ->
    $(elt).parent().nextAll('.column')
        .stop()
        .animate {width: 0}, 'fast', 'linear', () -> $(this).remove()
...
upToParent = (item) ->
    return if item.data('type') != 'comment'
